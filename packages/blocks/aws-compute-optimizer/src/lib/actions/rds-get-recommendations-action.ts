...
import {
  getRDSRecommendationsForARNs,
  getRDSRecommendationsForRegions,
} from '../common/compute-optimizer-rds-client';

export const rdsGetRecommendationsAction = createAction({
  auth: amazonAuth,
  name: 'rds_get_recommendations',
  description: 'Get RDS instance recommendations',
  displayName: 'RDS Get Recommendations',
  isWriteAction: false,
  props: {
    ...
    filterByARNs: Property.Checkbox({
      ...
      description:
        'If enabled, recommendations will be fetched only for resources that match the given ARNs',
      required: true,
      defaultValue: false,
    }),
    filterProperty: Property.DynamicProperties({
      ...
      props: async ({ filterByARNs }: { filterByARNs: boolean }) => {
  ...
  async run(context) {
    try {
      ...
      const promises = credentials.map((credentials) => {
        return getRDSRecommendationsForRegions(
          credentials,
          findingType,
          regions,
        );
      ...
      throw new Error(
        'An error occurred while requesting RDS recommendations: ' + error,
      );
    }
  },
});
